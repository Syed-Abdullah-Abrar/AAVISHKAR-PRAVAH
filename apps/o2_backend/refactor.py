...
# Instead of doing regex replacements which might break, we can just replace everything from LAKSHMI'S COMPLETE MEDICAL PROFILE
# up to the end of the file with the new logic.

start_marker = "# ─── LAKSHMI'S COMPLETE MEDICAL PROFILE (Demo Data) ──────────────────────────"

parts = code.split(start_marker)
header = parts[0]

new_code = header + start_marker + """
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    extractions = await process_smart_text(text, chat_id)
    if extractions:
        for ex in extractions:
            ...
            label = "Reading"
            emoji = "📝"
            if ex.get("type") == "bp_reading": label, emoji = "Blood Pressure", "🩸"
            elif ex.get("type") == "new_medication": label, emoji = "Medication", "💊"
            
            await save_patient_note(chat_id, ex.get("type", "note"), val)
...
with open("/home/syed/dev/AAVISHKAR-PRAVAH/apps/o2_backend/telegram_bot.py", "w", encoding="utf-8") as f:
    f.write(new_code)
